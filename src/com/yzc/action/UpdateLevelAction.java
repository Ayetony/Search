...
public class UpdateLevelAction extends ActionSupport {
	...
	public String execute() {
		...
		try {
			...
			if (null != deleteId){
				stmt.executeUpdate("delete from relation where relation.LevelId='"
						+ deleteId + "';");
				stmt.executeUpdate("delete from level where level.LevelId='"
						+ deleteId + "';");
				}
			else
				stmt.executeUpdate("insert into level values('" + createId
						+ "','" + createName + "');");
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	public void validate() {
		if(null == deleteId){
			if (null == createId || createId.length() == 0)
				this.addFieldError("createId", "权限Id不能为空！");
			if (null == createName || createName.length() == 0)
				this.addFieldError("createName", "权限名称不能为空！");
		}
	}
